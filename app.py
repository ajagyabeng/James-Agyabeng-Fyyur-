...
#  Venues
#  ----------------------------------------------------------------
def search_item(keyword, model):
    results = db.session.query(
        model.id, model.name).filter(model.name.ilike(f"%{keyword}%"))
    return {
        "count": results.count(),
        "data": [{
            "id": id,
            "name": name,
            "shows_upcoming": db.session.query(Show)
            .filter(Show.id == id, Show.start_time > datetime.now())
            .count()
...
def search_venues():
    ...
    return render_template('pages/search_venues.html', results=search_item(keyword, Venue), search_term=request.form.get('search_term', ''))
...
def show_venue(venue_id):
    ...
    upcoming_shows = db.session.query(Show).join(Venue).filter(
        Show.start_time > datetime.now(), venue_id == Show.venue_id)
    ...
    data = {
        ...
        # this is equivalent to past_shows in the past_shows
